...
from core.crawl import fetch_dantri_article
from core.formatByGemini import analyze_article_content, clean_gemini_json, save_processed_content
...
def main():
    ...
    try:
        ...
        print("Đang crawl dữ liệu từ URL...")
        raw_text, name = fetch_dantri_article(url)
        if not raw_text or not name:
            print("Không thể crawl dữ liệu. Thoát chương trình.")
            return

        # Bước 3: Chạy phân tích bằng Gemini
        print("Đang phân tích dữ liệu bằng Gemini AI...")
        structured_result = analyze_article_content(raw_text)
        if not structured_result:
            print("Lỗi khi phân tích dữ liệu. Thoát chương trình.")
            return

        cleaned_data = clean_gemini_json(structured_result)
        if not cleaned_data:
            print("Không thể làm sạch dữ liệu JSON. Thoát chương trình.")
            return
